...
def ppo_policy_error(data: namedtuple,
    ...
    **概述**:
    ...
    dist_new = [
        torch.distributions.categorical.Categorical(logits=logit)
        for logit in logit_new
    ]
    dist_old = [
        torch.distributions.categorical.Categorical(logits=logit)
        for logit in logit_old
    ]
    logp_new = torch.stack(
        [dist.log_prob(action[..., i]) for i, dist in enumerate(dist_new)], dim=-1
    )
    logp_old = torch.stack(
        [dist.log_prob(action[..., i]) for i, dist in enumerate(dist_old)], dim=-1
    )
    # 可选：如果需要总的 log_prob（所有动作的联合概率），可以对最后一维求和
    total_logp_new = logp_new.sum(dim=-1)
    total_logp_old = logp_old.sum(dim=-1)
    ...
    # dist_new 是一个包含多个 Categorical 分布的列表
    dist_new_entropy = torch.stack([dist.entropy() for dist in dist_new], dim=-1)

    # 如果需要总熵（所有分布的熵之和）
    total_entropy_new = dist_new_entropy.sum(dim=-1)
    entropy_loss = (total_entropy_new * weight).mean()
    # 重要性采样的权重: $$r(\theta) = \frac{\pi_{new}(a|s)}{\pi_{old}(a|s)}$$
    # ratio = torch.exp(logp_new - logp_old)
    ratio = torch.exp(total_logp_new - total_logp_old)
